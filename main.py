...
def upload_file():
    ...
    return jsonify(message=f'Files uploaded successfully. Access them at: ', link=filelink), 201   
...
def download_file(link, filename):
    ...
    cursor.execute(f"SELECT file_name FROM {TABLE_NAME} WHERE file_link = %s AND file_name = %s", (link, filename))
